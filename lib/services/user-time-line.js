...
exports.UserTimeLine = void 0;
const sentiment_analysis_1 = require("./sentiment-analysis");
class UserTimeLine {
    constructor(twitterApi, userAccount) {
    ...
    async timeLineAnalysis(numberOfTweet, tweetUserTimelineV2Paginator) {
        if (!tweetUserTimelineV2Paginator) {
            tweetUserTimelineV2Paginator = await this.getMinimalTimeLine(numberOfTweet);
        }
        const userTimeLineSentimentAnalysis = Promise.all(tweetUserTimelineV2Paginator.data.data.map((tweet) => (0, sentiment_analysis_1.processingSentiment)(tweet.text)));
        return ((await userTimeLineSentimentAnalysis).reduce((pv, vp) => pv + vp.score, 0)) / (await userTimeLineSentimentAnalysis).length;
    }
    async getPositionIfExist(numberOfTweet = 1000) {
        const geoReturn = [];
